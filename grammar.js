...
module.exports = grammar({
  ...
  rules: {
    ...
    basic_type: ($) =>
      choice("int", "float", "string", "boolean", "map", $.nil_literal),
    nil_literal: ($) => choice("null", "()"),
    _union_type: ($) =>
      prec.left(seq($._type_descriptor, "|", $._type_descriptor)),
    _intersection_type: ($) =>
      prec.left(seq($._type_descriptor, "&", $._type_descriptor)),
    ...
    array_type: ($) =>
      prec.left(
        seq($._type_descriptor, $._array_dimension, repeat($._array_dimension)),
      ),
    ...
    _tuple_members: ($) =>
      choice(
        seq(
          $._type_descriptor,
          repeat(prec.left(seq(",", $._type_descriptor))),
    ...
    record_type: ($) =>
      choice($._inclusive_record_type_desc, $._exclusive_record_type_desc),
    _inclusive_record_type_desc: ($) =>
      seq("record", "{", repeat($._field_desc), "}"),
    _exclusive_record_type_desc: ($) =>
      seq(
        "record",
        "{|",
        repeat($._field_desc),
        optional($._record_rest),
        "|}",
      ),
    _field_desc: ($) => choice($._individual_field_desc, $._type_inclusion),
    // TODO: add default expressions
    _individual_field_desc: ($) =>
      seq(
        optional("readonly"),
        $._type_descriptor,
        $.identifier,
    ...
    function_type: ($) =>
      seq(optional($._quals), "function", $._function_signature),
    _function_signature: ($) =>
      seq(
        "(",
        optional($._function_params),
        ")",
        optional($._return_type_desc),
      ),
    ...
    _function_params: ($) =>
      prec.right(seq($._function_param, repeat(seq(",", $._function_param)))),
    _function_param: ($) =>
      choice($._required_param, $._included_record_param, $._rest_param),
    _required_param: ($) => seq($._type_descriptor, optional($.identifier)),
    _included_record_param: ($) =>
      seq("*", $._type_reference, optional($.identifier)),
    _rest_param: ($) => seq($._type_descriptor, "...", optional($.identifier)),
    _return_type_desc: ($) => prec.right(seq("returns", $._type_descriptor)),

    object_type: ($) =>
      seq(optional($._quals), "object", "{", repeat($._object_member), "}"),
    _object_member: ($) =>
      choice(
        $.object_field,
        $.method_decl,
    ...
    object_field: ($) =>
      seq(optional("public"), $.identifier, $._type_descriptor, ";"),
    method_decl: ($) =>
      seq(
        optional("public"),
        optional($._quals),
